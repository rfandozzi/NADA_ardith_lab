...
class SpaDatasetRaster:
	...
	def GetMinMax(self,Index=0):
		...
		"""
		self.GetBands() # problem with mask not being setup on load? jjg
		...
		if (self.TheMask is not None): # if there is a mask, use it
			# This code did not work and I'm not sure what is being returned so I replaced it with Python code (jjg)
...
#			Max=numpy.ma.MaskedArray.max(TheBand)
			...
			YIndex=0
			while (YIndex<self.HeightInPixels):
				XIndex=0
				while (XIndex<self.WidthInPixels):
					
					if (self.TheMask[YIndex][XIndex]==False):
						
						Value=TheBand[YIndex][XIndex]
						if Min==None:
							Min=Value
							Max=Value
						else:
	...
	def SetType(self,GDALDataType):
		...
		if (self.GDALDataset is not None):
			self.GDALDataset = gdal.Translate('', self.GDALDataset, format="MEM",outputType=GDALDataType)
	...
	def GetHistogram(self,BandIndex=0,NumBins=10):
		...
		Returns:
			...
			Returns None if there are no valid pixels in the raster
			NumPy is used unless there is a mask and then the calc is done in Python (slower)
		...
		#self.GetBands() # jjg problem with mask not being computed on load?
		...
		if (self.TheMask is not None): # if there is a mask, use it
			...
			TheMinMax=self.GetMinMax()
			
			if (TheMinMax!=None):
				#Get the min/max values
				MinValue=TheMinMax[0]
				MaxValue=TheMinMax[1]
...
#					if (ValuesInitialized):
				Increment=(MaxValue-MinValue)/NumBins
				Result=[0] * NumBins
				...
				while (YIndex<self.HeightInPixels):
					...
					while (XIndex<self.WidthInPixels):
						...
						if (self.TheMask[y][x]==False):
							
							Value=TheBand[y][x]
							Index=math.floor((Value-MinValue)/Increment)
							Result[Index]+=1
					
		# Numpy histogram function does not support masks
