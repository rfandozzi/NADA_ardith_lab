...
#
# Changes:
# 12/28/2023 - added support to read geojson files.  Need to add code to convert geojson crs to osr
# 12/31/2023 - added SetGeometry() function
#
...
class SpaDatasetVector:
	...
	def Load(self,FilePath):
		...
		TheExtension=FilePathParts[1]
		TheExtension.lower()
		
		if (TheExtension==".geojson"):
			import json
			...
			TheJSON=json.loads(TheText)
			
			TheType=TheJSON["type"]
#			TheName=TheJSON["name"]
#			TheType=TheJSON["type"]
			
#			TheCRSName=TheJSON["name"]
			
			TheFeatures=TheJSON["features"]

			################################################################
			if 'crs' in TheJSON.keys():
				TheCRS=TheJSON["crs"]
				
				TheProperties=TheCRS["properties"]
				# setup the spatial reference
				#spatialRef = osr.SpatialReference()
				#spatialRef.ImportFromUrl(TheCRS)
			...
			while (FeatureIndex<NumFeatures):
				TheFeature=TheFeatures[FeatureIndex]
				
				TheFeatureProperties=TheFeature["properties"]
				TheProperyKeys=TheFeatureProperties.keys()
				TheProperyKeys=list(TheProperyKeys)
				
				KeyIndex=0
				while (KeyIndex<len(TheProperyKeys)):
					TheKey=TheProperyKeys[KeyIndex]
					TheValue=TheFeatureProperties[TheKey]
					
					# Find the attribute name in the attribute defs
					AttributeDefIndex=0
					MatchIndex=-1
					while (AttributeDefIndex<len(self.AttributeDefinitions)):
						if (self.AttributeDefinitions[AttributeDefIndex]["Name"]==TheKey):
							MatchIndex=AttributeDefIndex
						AttributeDefIndex+=1
					
					# if not found, add the new attribute definition
					if (MatchIndex==-1):
						TypeName="String" # "Integer" "Float" "Boolean" "Date"
						Precision=10
						Width=254
						...
						# if the attribute is not a number and the def is float, we need to convert it back to string
						if (isinstance(TheValue,str)):
							if (self.AttributeDefinitions[MatchIndex]["Type"]!="String"):
								self.AttributeDefinitions[MatchIndex]["Type"]="String"
								self.AttributeDefinitions[MatchIndex]["Width"]=10
								self.AttributeDefinitions[MatchIndex]["Precision"]=254
								
						elif (isinstance(TheValue,float)): # promote int to float
							if (self.AttributeDefinitions[MatchIndex]["Type"]=="Integer"):
								self.AttributeDefinitions[MatchIndex]["Type"]="Float"
								self.AttributeDefinitions[MatchIndex]["Width"]=16.6 
								self.AttributeDefinitions[MatchIndex]["Precision"]=254
					KeyIndex+=1
			...
			while (FeatureIndex<NumFeatures):
				TheFeature=TheFeatures[FeatureIndex]
				TheFeatureType=TheFeature["type"]
				
				TheGeometry=TheFeature["geometry"]
				TheGeometry = shapely.geometry.shape(TheGeometry)
			...
			while (FeatureIndex<NumFeatures):
				...
				# because we cannot guarenttee the attributes are all in the geojson file (or their order)
				# we initialize all the attributes as none
				TheRow=[]
				for x in range(NumAttributes): TheRow.append(None)
					
				self.TheAttributes.append(TheRow)
				...
				while (KeyIndex<len(TheProperyKeys)):
					...
					TheAttributeIndex=self.GetAttributeIndexFromName(TheKey)
					
					self.SetAttributeValue(TheAttributeIndex,FeatureIndex,TheValue)
					
					KeyIndex+=1
					
				FeatureIndex+=1
		################################################################
		else: # assume the file is a shapefile
	...
	def Save(self,FilePath,GeomteryType=None):
		...
		if (TheExtension==".geojson"):
			##################################################################
			# write out a geojson file (jjg - we probably need file converter functions or classes)
			...
			while (AttributeIndex<NumAttributes):
				...
				TheAttributeNames.append(TheName)
				
				AttributeIndex+=1
				
			# setup the JSON dictionary
			TheJSON={
				"type":"FeatureCollection"
			}
			
			TheFeatures=[]
			TheJSON["features"]=TheFeatures
			...
			while (FeatureIndex<NumFeatures): # write each feature one at a time
				...
				TheGeometry=shapely.to_geojson(TheGeometry) # convert the shapely to GeoJSON text
				TheGeometry = json.loads(TheGeometry) # conver the text to GeoJSON campatible object
				...
				# setup properties
				TheProperties={}
				TheFeature[ "properties"]=TheProperties
				
				# add attributes as properties in GeoJSON
				TheRow=self.TheAttributes[FeatureIndex]
				
				AttributeIndex=0
				while (AttributeIndex<len(TheAttributeNames)):
					
					TheValue=TheRow[AttributeIndex]
					
					TheProperties[TheAttributeNames[AttributeIndex]]=TheValue
					
					AttributeIndex+=1
					
				FeatureIndex+=1
				
			TheText=json.dumps(TheJSON)
...
#				TheGeometry=shapely.geometry.GeometryCollection([TheGeometry])
	...
	def SetGeometry(self,Index,TheGeometry):
		"""
		Returns the shapely geometry for the specified feature
		...
		Returns:
			Shapely geometry for the feature.
		"""
		self.TheGeometries[Index]=TheGeometry
